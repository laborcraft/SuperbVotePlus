...
public class ScriptRewardMatcher implements RewardMatcher {
    ...
    public ScriptRewardMatcher(Path path) throws IOException, ScriptException {
        ...
        Thread.currentThread().setContextClassLoader(SuperbVotePlus.getPlugin()._exposeClassLoader());
        try {
            ScriptEngineManager manager = new ScriptEngineManager();
            engine = manager.getEngineByName("JavaScript");

            // Read SuperbVotePlus helper library first
            try (Reader reader = new BufferedReader(new InputStreamReader(SuperbVotePlus.getPlugin().getResource("superbvote_lib.js")))) {
                engine.eval(reader);
            }
    ...
    public boolean matches(Vote vote, PlayerVotes pv) {
        ...
        try {
            ...
            SuperbVotePlus.getPlugin().getLogger().log(Level.WARNING, "Unable to execute 'matchVote' function in " + path, e);
            return false;
        }
    }
