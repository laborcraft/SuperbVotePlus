    ...
    public static List<RewardMatcher> getMatchers(ConfigurationSection section) {
        ...
        if (!unknownKeys.isEmpty()) {
            for (String key : unknownKeys) {
                ...
                if (closestMatches.size() == 1) {
                    SuperbVotePlus.getPlugin().getLogger().warning("Unknown matcher '" + key + "'. Perhaps you meant '" + closestMatches.get(0) + "'?");
                } else if (closestMatches.size() >= 1){
                    SuperbVotePlus.getPlugin().getLogger().warning("Unknown matcher '" + key + "'. Try one of these instead: " + Joiner.on(", ").join(closestMatches));
                } else {
                    SuperbVotePlus.getPlugin().getLogger().warning("Unknown matcher '" + key + "'. Check your spelling.");
                }
            }
        }
        ...
        for (RewardMatcherFactory factory : MATCHER_FACTORIES) {
            ...
            try {
                ...
                SuperbVotePlus.getPlugin().getLogger().severe("Invalid matcher found: " + e.getMessage());
                matcher = Optional.of(StaticRewardMatcher.ERROR);
            } catch (Exception e) {
                SuperbVotePlus.getPlugin().getLogger().log(Level.SEVERE, "Unable to create matcher", e);
                matcher = Optional.of(StaticRewardMatcher.ERROR);
            }
