...
public class TopPlayerSignFetcher implements Runnable {
    ...
    public void run() {
        ...
        List<PlayerVotes> topPlayers = SuperbVotePlus.getPlugin().getVoteStorage().getTopVoters(toFetch.getAsInt(), 0);

        // We've done everything we can do asynchronously. Hand off to the synchronous update task.
        Bukkit.getScheduler().runTask(SuperbVotePlus.getPlugin(), new TopPlayerSignUpdater(toUpdate, topPlayers));
    }
}
