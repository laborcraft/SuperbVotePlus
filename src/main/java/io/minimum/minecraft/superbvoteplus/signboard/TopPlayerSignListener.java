...
public class TopPlayerSignListener implements Listener {
    ...
    private boolean doSignBreak(Player player, TopPlayerSign sign) {
        ...
        SuperbVotePlus.getPlugin().getTopPlayerSignStorage().removeSign(sign);
    ...
    public void onSignChange(SignChangeEvent event) {
        try {
            if (event.getLine(0).startsWith("[topvoter]") && event.getPlayer().hasPermission("superbvote.managesigns")) {
                ...
                SuperbVotePlus.getPlugin().getTopPlayerSignStorage().addSign(sign);
                updateSigns();
    ...
    private void updateSigns() {
        Bukkit.getScheduler().runTaskAsynchronously(SuperbVotePlus.getPlugin(), new TopPlayerSignFetcher(
                SuperbVotePlus.getPlugin().getTopPlayerSignStorage().getSignList()));
    }
}
