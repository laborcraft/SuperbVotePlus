package io.minimum.minecraft.superbvoteplus.scoreboard;
...
public class ScoreboardHandler {
    ...
    public void reload() {
        objective.setDisplayName(ChatColor.translateAlternateColorCodes('&',
                SuperbVotePlus.getPlugin().getConfig().getString("leaderboard.scoreboard.title", "Top voters")));
    }

    public void doPopulate() {
        if (!SuperbVotePlus.getPlugin().getConfig().getString("leaderboard.display").equals("scoreboard")) {
            return;
        }
        List<PlayerVotes> leaderboardAsUuids = SuperbVotePlus.getPlugin().getVoteStorage().getTopVoters(
                Math.min(16, SuperbVotePlus.getPlugin().getConfig().getInt("leaderboard.scoreboard.max", 10)), 0);
