...
public class MysqlVoteStorage implements ExtendedVoteStorage {
    ...
    public void initialize() {
        ...
        YamlConfiguration dbInfo = YamlConfiguration.loadConfiguration(new File(SuperbVotePlus.getPlugin().getDataFolder(), "db_version.yml"));
        dbInfo.options().header("DO NOT EDIT - SuperbVotePlus internal use");
        ...
        if (!readOnly) {
            try (Connection connection = dbPool.getConnection()) {
                try (ResultSet t = connection.getMetaData().getTables(null, null, tableName, null)) {
                    if (!t.next()) {
                        ...
                        if (ver < TABLE_VERSION_CURRENT) {
                            SuperbVotePlus.getPlugin().getLogger().log(Level.INFO, "Migrating database from version " + ver + " to " + TABLE_VERSION_CURRENT + ", this may take a while...");
                ...
                SuperbVotePlus.getPlugin().getLogger().log(Level.SEVERE, "Unable to initialize database", e);
            }

            if (isUpdated) {
                dbInfo.set("db_version", TABLE_VERSION_CURRENT);
                try {
                    dbInfo.save(new File(SuperbVotePlus.getPlugin().getDataFolder(), "db_version.yml"));
                } catch (IOException e) {
                    SuperbVotePlus.getPlugin().getLogger().log(Level.SEVERE, "Unable to save DB info", e);
                }
            }
        }
    }
    ...
    public void clearVotes() {
        ...
        try (Connection connection = dbPool.getConnection()) {
            ...
            SuperbVotePlus.getPlugin().getLogger().log(Level.SEVERE, "Unable to clear votes", e);
        }
    }
    ...
    public PlayerVotes getVotes(UUID player) {
        ...
        try (Connection connection = dbPool.getConnection()) {
            ...
            SuperbVotePlus.getPlugin().getLogger().log(Level.SEVERE, "Unable to get votes for " + player.toString(), e);
            return new PlayerVotes(player, null, 0, PlayerVotes.Type.CURRENT);
        }
    }
    ...
    public List<PlayerVotes> getTopVoters(int amount, int page) {
        ...
        try (Connection connection = dbPool.getConnection()) {
            ...
            SuperbVotePlus.getPlugin().getLogger().log(Level.SEVERE, "Unable to get top votes", e);
    ...
    public int getPagesAvailable(int amount) {
        try (Connection connection = dbPool.getConnection()) {
            ...
            SuperbVotePlus.getPlugin().getLogger().log(Level.SEVERE, "Unable to get top votes page count", e);
            return 0;
        }
    }
    ...
    public List<PlayerVotes> getAllPlayersWithNoVotesToday(List<UUID> onlinePlayers) {
        ...
        try (Connection connection = dbPool.getConnection()) {
            ...
            SuperbVotePlus.getPlugin().getLogger().log(Level.SEVERE, "Unable to batch-get votes", e);
            return ImmutableList.of();
        }
    }
    ...
    public VoteStreak getVoteStreak(UUID player, boolean required) {
        ...
        try (Connection connection = dbPool.getConnection()) {
            ...
            SuperbVotePlus.getPlugin().getLogger().log(Level.SEVERE, "Unable to get or reset vote streak for " + player.toString(), e);
            return new VoteStreak(player, 0, 0, Maps.newHashMap());
        }
    }
